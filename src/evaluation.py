...
def evaluate_sessions_batch(pr, train_data, test_data, items=None, cut_off=20, batch_size=100, break_ties=False,
    ...
    """
    Evaluates the GRU network wrt. recommendation accuracy measured by recall@N and MRR@N.

    Parameters
    --------
    pr : gru.GRU
        A trained instance of the GRU network.
...
def evaluate_sessions_batch_hier_bootstrap(pr, train_data, test_data, items=None, cut_off=20, batch_size=100,
    ...
    """
    Evaluates the GRU network wrt. recommendation accuracy measured by recall@N and MRR@N.
    Concatenates train sessions to test sessions to bootstrap the hidden states of the GRU.
	...
	#print(test_data[session_key].values[session_start_valid])


	l = test_data[session_key].values[session_start_valid]
	session_list_int = pd.Series(l).astype('category').cat.codes.values
	
        in_session_id[valid_mask] = session_list_int
